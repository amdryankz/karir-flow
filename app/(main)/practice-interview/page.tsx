...
export default function PracticeInterviewPage() {
  ...
  if (isPending || loading) {
    return (
      <div className="min-h-screen bg-background p-6 md:p-12 font-sans transition-colors duration-300">
        <div className="mx-auto max-w-6xl space-y-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-medium tracking-tight text-foreground">My Interviews</h1>
  ...
  return (
    <div className="min-h-screen bg-background p-6 md:p-12 font-sans text-foreground transition-colors duration-300">
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <Button asChild className="h-10 rounded-full px-6 font-medium shadow-sm transition-all hover:shadow-md">
            <Link href="/practice-interview/start">Start New Interview</Link>
          </Button>
        </div>

        <Card className="overflow-hidden border-none shadow-md bg-card rounded-2xl">
          <CardHeader className="border-b bg-muted/50 px-8 py-6">
            <CardTitle className="text-xl font-medium text-foreground">Recent Sessions</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {interviews.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-medium text-foreground">No interviews yet</h3>
                <p className="mb-6 max-w-sm text-muted-foreground">
                  Start your first practice session to get AI-powered feedback on your answers.
                </p>
                <Button asChild variant="outline" className="rounded-full text-primary hover:text-primary hover:bg-muted">
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow className="hover:bg-transparent border-b">
                    <TableHead className="w-[60px] px-8 py-4 font-medium text-muted-foreground">No</TableHead>
                    <TableHead className="py-4 font-medium text-muted-foreground">Title</TableHead>
                    <TableHead className="py-4 font-medium text-muted-foreground">Date</TableHead>
                    <TableHead className="py-4 font-medium text-muted-foreground">Status</TableHead>
                    <TableHead className="py-4 font-medium text-muted-foreground">Score</TableHead>
                    <TableHead className="px-8 py-4 text-center font-medium text-muted-foreground">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {interviews.map((interview, index) => {
                    ...
                    return (
                      <TableRow key={interview.id} className="border-b hover:bg-muted/50 transition-colors">
                        <TableCell className="px-8 py-5 font-medium text-foreground">{index + 1}</TableCell>
                        <TableCell className="py-5 font-medium text-foreground">{interview.title}</TableCell>
                        <TableCell className="py-5 text-muted-foreground">{formatDate(interview.startedAt)}</TableCell>
                        ...
                        <TableCell className="px-8 py-5 text-center space-x-2">
                          ...
                          {interview.finishedAt || (interview.answers && interview.answers.length > 0) ? (
                            <>
                              <Button variant="outline" size="sm" asChild className="rounded-full hover:border-primary hover:text-primary">
                                <Link href={`/practice-interview/result?id=${interview.id}`}>
                                  <Eye className="h-4 w-4 mr-2" />
                                  Results
                                </Link>
                              </Button>
                              <Button variant="outline" size="sm" asChild className="rounded-full text-primary hover:border-primary hover:text-primary">
                            ...
                            <Button variant="outline" size="sm" asChild className="rounded-full border-primary text-primary hover:bg-muted">
